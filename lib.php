 ...
 *
 * @package    theme_tkkp
 * @copyright 2023 Yohan Naftali - http://yohanli.com
 ...
 * @return string
 */
function theme_tkkp_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_tkkp', 'preset', 0, '/', $filename))) {
    ...
    // tkkp scss.
    $tkkpvariables = file_get_contents($CFG->dirroot . '/theme/tkkp/scss/tkkp/_variables.scss');
    $tkkp = file_get_contents($CFG->dirroot . '/theme/tkkp/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/tkkp/scss/tkkp/_security.scss');

    // Combine them together.
    $allscss = $tkkpvariables . "\n" . $scss . "\n" . $tkkp . "\n" . $security;
 ...
 * @return string
 */
function theme_tkkp_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/tkkp/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_tkkp_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_tkkp_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/tkkp/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_tkkp_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('tkkp');
